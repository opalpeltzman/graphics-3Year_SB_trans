...
class myWindowApp():
    ...
    def initWindow(self):
        ...
        transform_menu.add_command(label="Scaling", command=self.scalling_transformation)
        transform_menu.add_separator()
        transform_menu.add_command(label="Rotation", command=self.rotate)
        transform_menu.add_separator()
        transform_menu.add_command(label="Translation", command=self.translation)
        transform_menu.add_separator()
        ...
        x_entry = Entry(self.tollbar)
        x_entry.pack(side=LEFT, padx=2, pady=2)
        y_value = Label(self.tollbar, text="enter Y value")
        y_value.pack(side=LEFT, padx=2, pady=2)
        y_entry = Entry(self.tollbar)
        y_entry.pack(side=LEFT, padx=2, pady=2)
        send = Button(
            self.tollbar,
            relief=FLAT,
            compound=LEFT,
    ...
    def draw_curve(self, x1, y1, x2, y2, x3, y3, x4, y4):
        ''' This function draws curve given 4 points based on Bezier algorithm for drawing curve '''
        linesInCurve = 1000
        dt = 1 / linesInCurve
